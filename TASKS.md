...
| X08 | Language pass over every user-facing text before release 0.1.0: the translation sources (`translations_src/base.*.json` and every feature fragment) in German and English, and the user pages under `docs/features/`. Keys, placeholders and parity stay unchanged; only the wording changes. The texts describe the effect for a person who can operate Home Assistant and knows nothing of the inner workings, never the mechanism, without words of the data model; German is written in its own right with the informal address of the German Home Assistant interface, English follows the style of the Home Assistant interface. A glossary of the fixed terms in both languages is added to the repository so that later blocks and the user manual (W01) use the same vocabulary. The reviewer checks readability for a non-technical reader, consistency with the glossary, unchanged placeholders, and that no text claims a behaviour the integration does not have; the project owner proof-reads both languages | done (pull request 67) |
| X09 | Form behaviour before release 0.1.0, from the project owner's review of the house flow on a throw-away instance (2026-09-25), one small block with a reviewer, sequence numbers per commit. (1) Range checks: the morning and evening positions accept -100 and 200 and fail only on submit with the raw text "Value -100.0 is too small"; for every field with a range (positions, percentages, durations, offsets, illuminance) a number selector with its bounds, a server-side check with a translated error key instead of raw text, a test that no untranslated validation text reaches the user, and the float rendering "-100.0" checked. (2) Time fields: a time selector without seconds for every time of day; saved times with seconds must still load. (3) Multi-page flows: intermediate pages marked as not the last step, so the frontend shows "Next" instead of "Submit"; the last page may carry its own submit text; the daily-routine titles carry a counter as a placeholder ("Daily routine: workdays (1/3)") with the count following the enabled kinds of day. Verified by the owner in the frontend source: the config-flow and sub-config-flow dialogs read the step's own submit text, otherwise next or submit by the last-step flag. (4) Language, in addition to X08: the introduction page also says that every setting can be changed later through "Reconfigure", because the flow has no way back and the user must know that | in progress |
