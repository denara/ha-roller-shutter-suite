...
Where a plausible own name had the shape of a private value, the guard was narrowed, never the name: the fixed names of high-resolution brand images (`icon@2x.png` and the like: a scale factor and an image extension behind the `@` are no mail domain); a folder called `home` inside another folder (`docs/home/index.md`; a home directory starts a path or follows a quote, a space or `=`); a version with a marker letter in front (`v10.1.2.3`); a Python slice with a start and a step (`items[1::2]`; a decimal start of at most three digits before the double colon names an address in a reserved block, never an installation); and, in a Python file only, an attribute called `.local` outside of strings and comments, which the tokenizer tells from a host name in a string, a comment or a docstring. Left as they are, because no shape tells them from the real thing: a bare file name `<word>.local` (a settings file marks a local variant in front of its real extension, `settings.local.json`, and passes), a four-part number behind a hyphen in a name (`upgrade-10.1.2.3.md`), and a long hexadecimal part behind an entity domain, which is what a serial number looks like. These are reported and stay the documented exception.
