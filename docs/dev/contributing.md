...
Some assistant tools append a link to their own session or conversation to a commit message by themselves. Such a link leads to a private page of whoever worked there, so the guard flags it, in commit messages and in files alike. The pattern is narrow on purpose: the host of a known tool, followed by the part of the path under which that tool keeps sessions, conversations or shared conversations, followed by an identifier (`claude.ai/code/<session>`, `claude.ai/chat/<id>`, `claude.ai/share/<id>`, the `/c/<id>` and `/share/<id>` pages of `chatgpt.com` and `chat.openai.com`, `gemini.google.com/app/<id>` and its share links, the chat pages of `copilot.microsoft.com` and of `github.com/copilot`). A plain link to a product is not flagged; the attribution line `Generated with [Claude Code](https://claude.com/claude-code)` and the `Co-Authored-By` trailer with the address at `anthropic.com` pass. Remove a flagged link from the message before you commit; if a tool adds it by itself, switch that off in the tool.
