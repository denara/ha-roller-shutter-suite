...
| `OwnCommand` | A command of the integration to a member: its identifier, target, direction (`TravelDirection`: `up` or `down`), time, wish class, the reason code of the wish that caused it, and the context under which it was executed once that is known, and whether the call to the actuator failed (`failed`, from a `command_failed` result: the record stays, but gate rule 3 never counts a failed command as "the last own command already had this target"). The reason is a code of the group "winning or contributing layers", it is persisted, and it is authoritative for what the movement is: a decision documents a moment, the command says whose movement this is now. When a wish of a higher class takes a movement in flight over, class and reason of the command become those of that wish. The identifier goes to the actuator and comes back with the result, so a late result is never attributed to a newer command. |
| `CommandResult` | What the actuator reports back about one own command: command identifier, member, the context ID of the call (or none if nothing was called) and `failed`. `Engine.on_command_result(state, result)` writes it into the last own command of the member, and only if the identifiers match. A result says only whether the actuator accepted the command, never that a curtain arrived. |
