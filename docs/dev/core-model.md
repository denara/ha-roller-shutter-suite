...
| `SunAlmanac`, `SunDay`, `ElevationPassage` | The answers of the sun port for a few local dates: sunrise, sunset, and when the sun passes the elevations the schedule of the window names. "None on this day" is an answer; what was never asked is missing and is never guessed. Instants are kept in UTC; `to_data()` and `from_data()` turn it into plain data and back. `build_sun_almanac` of the schedule fills it. |
