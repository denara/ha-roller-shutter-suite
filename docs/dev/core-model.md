...
| `functions` | the closed list of function identifiers, each with its fault behavior |
...
| `Decision` | The complete result of one recompute: the winning wish, the reasons of the other layers, the constraint results, the target of every member, the gate outcome, and `paused_functions`: the functions whose layer, or part of a layer, was not asked because a faulty stored setting has disabled the function for the window. All of them name the same members in the same order. `target` is the target the window shows: the common target if all members have the same one, otherwise none, with the per-member targets as the detail. |
...
| `WindowConfig` | A window after inheritance has been resolved: identifier, covering type, members, and three places for later features (the condition input of the morning opening, the tiers of the temperature condition, the profile key of the schedule). The blocks that build a feature add its settings here; so far `motor_protection`, `frost`, `reevaluate_after`, the upper bound of a deferral whose end is not known (default 5 minutes), and `disabled_functions`, the functions that are paused for the window because a stored setting of theirs is faulty (a frozen set of `FunctionId`, empty by default; only functions with the fault behavior `pause` are accepted; the arbiter does not ask the layer of a listed function). |
| `FunctionId`, `FaultBehavior` | The closed list of the functions of the integration (`schedule`, `shading`, `frost` …) and what happens to each when one of its stored settings is faulty (`FunctionId.fault_behavior`): a function that creates wishes is paused (`pause`), a function that restricts movement falls back to the next level and keeps working (`fall_back`), because pausing a restriction would mean more movement. The settings registry, the disabled functions of a window and the registrations of the arbiter all use the enumeration; there are no free strings. The member list is provisional: a block that builds a function adds its member with its fault behavior. |
