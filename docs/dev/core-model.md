...
| `geometry` | the measurements of shading as values: the window-level measurements, the glass of a member, the glass calibration, and their rules |
...
| `MemberConfig` | One member: its identifier, its capability profile, and `measurements`, what the member states itself for geometric shading (`MemberMeasurements`; by default nothing, so it inherits the window's). The inheritance resolver fills `measurements` from the member-level settings, with sound values only; measurements on a member that is handed to `resolve_window` are dropped. |
...
| `WindowConfig` | A window after inheritance has been resolved: identifier, covering type, members, and three places for later features (the condition input of the morning opening, the tiers of the temperature condition, the profile key of the schedule). The blocks that build a feature add its settings here, one field per setting so that each can be inherited on its own. Frost protection: `frost_source` (the key of the temperature source; none means not configured; `BLIND_SOURCE` means configured, but blind, see the next row), `frost_threshold` (default 0), `frost_hysteresis` (default 1), `frost_position` (default 90), `frost_applies_to_protection` (default no) and `frost_hold_closed`, the option "do not raise a closed window at all" (default off); `frost` is the view over them. Motor protection: `motor_min_change` in percent (default 5) and `motor_min_interval` between two own comfort movements (default 10 minutes), zero switches a part off; `motor_protection` is the view over them. `reevaluate_after` is the upper bound of a deferral whose end is not known (default 5 minutes). Staggering between motors (E13): `stagger_gap` (default 2 seconds, 0 to 10 seconds), the time a collective movement waits after each motor of this window, between windows and between its own members; zero switches staggering off for the motors of this window. It belongs to motor protection, the core only carries it, and the actuator adapter of the Home Assistant layer applies it. The schedule: `schedule_enabled` and the other `schedule_*` fields, among them six fields per day type and edge (`schedule_workday_morning_time` …); [the schedule](schedule.md#settings) lists them with kinds and defaults, and `schedule` is the view over them. Two rules span several of them (clamps in order for a trigger of the sun; the latest morning before the earliest evening) and raise `SettingsCombinationError` with the keys they concern. The measurements of geometric shading: `shading_use_measurements` (default off: the simple mode with `shading_fixed_position`, default 30), `shading_orientation_known` (default off; the inheritance resolver lets it be on only if a level really states `shading_orientation`, because the built-in azimuth is a number nobody entered, and it reports no fault for that) and the other `shading_*` measurements; [Sun geometry and glass calibration](geometry.md#settings) lists them with kinds, ranges and defaults, and `geometry` is the view over them. Every one of them always has a value; "no measurements" is said by the two switches. One rule spans two of them (the calibration position at the upper glass end lies at least 10 above the one at the seating point). `disabled_functions` is the set of functions (`FunctionId`) that are paused for this window because a stored setting of theirs is faulty; the inheritance resolver fills it, it is never stored, and the arbiter skips the layers of such a function and says so in the decision. Only a function whose fault behavior is `pause` is accepted; any other member, and anything that is no `FunctionId`, is refused on construction: whatever protects or restricts movement can never be switched off by a data fault. |
...
| `ShadingGeometrySettings` | The window-level measurements of geometric shading as one value: a read-only view over the `shading_*` measurement fields of `WindowConfig` (`WindowConfig.geometry`), with their value rules, units and conventions. `GeometryRuleError` is what the view raises for its rule over several settings; `WindowConfig` turns it into a `SettingsCombinationError` with the names of its fields. `GEOMETRY_FIELDS` and `GEOMETRY_PREFIX` are the one list from which the fields, the view and the registry entries follow. [Sun geometry and glass calibration](geometry.md) explains them. |
| `MemberMeasurements`, `MemberGlassError` | What one member states itself: glass height, top offset and the two calibration positions, each `None` while it is not stated (`None` is no value of any of them; `NO_MEASUREMENTS` states nothing). `member_glass_for(window, member_id, stated)` returns the glass that applies, a member's own value or else the window's, and raises `MemberGlassError` with the member and the fields it states when the values do not fit together (calibration points in order and apart; the glass inside the element). `WindowConfig.member_glass` is the view over all members, and a `WindowConfig` whose members do not fit is refused. `MEMBER_MEASUREMENT_FIELDS` are the four names, which are also the keys of the member-level settings. |
| `GlassCalibration`, `MemberGlass` | The glass calibration of one motor (the positions at the seating point and at the upper glass end; `NO_CALIBRATION` is the full range), and the glass of one member inside the element (glass height, offset of its top edge, calibration). The functions that compute with them live in `core/geometry`. |
...
| `OwnCommand` | A command of the integration to a member: its identifier, target, direction (`TravelDirection`: `up` or `down`), time, wish class, the reason code of the wish that caused it, and the context under which it was executed once that is known, and whether the call to the actuator failed (`failed`, from a `command_failed` result: the record stays, but gate rule 3 never counts a failed command as "the last own command already had this target"; within schema version 1 the key is optional when data is read and missing means not failed, the schema step with a migration belongs to the persistence block). The reason is a code of the group "winning or contributing layers", it is persisted, and it is authoritative for what the movement is: a decision documents a moment, the command says whose movement this is now. When a wish of a higher class takes a movement in flight over, class and reason of the command become those of that wish. The identifier goes to the actuator and comes back with the result, so a late result is never attributed to a newer command. |
| `CommandResult` | What the actuator reports back about one own command: command identifier, member, the context ID of the call (or none if nothing was called), `failed`, and `called_at`, the instant of the call (none if nothing was called). `Engine.on_command_result(state, result)` writes it into the last own command of the member, and only if the identifiers match; for an accepted command with `called_at` it moves the time of the command and of its attempt to that instant, so the expectation window of a staggered command starts when the cover was really commanded. The motor protection clock stays at the hand-over. A result says only whether the actuator accepted the command, never that a curtain arrived. |
...
**The one implementation of the sun port.** Sun times and positions are computed in exactly one place, `AstralSun` in `custom_components/roller_shutter_suite/sun_astral.py`, on top of the `astral` library. The runtime (block H02) and the time-lapse simulation (block C05) both use it, so a simulated year runs with the same sunrises, sunsets and elevation passages as the operation, and `build_sun_almanac` of the schedule stays the one place that fills the almanac. The module lives outside `core/` on purpose: the domain core imports no third-party library, so `core/` holds only the port (the `Sun` protocol), the almanac and the builder, and `sun_astral.py` is the adapter between the two. The module itself imports nothing from Home Assistant and takes plain values (latitude, longitude, elevation above sea level, and the name of the local zone). Importing it **through the integration package** still executes the package's `__init__`, which belongs to the Home Assistant layer and imports Home Assistant; an importer that has to stay free of Home Assistant registers an empty stand-in for the package first, as `tests/core/conftest.py` and the command line of the time-lapse simulation do. `astral` is not listed as a requirement of the integration, because Home Assistant Core ships it as one of its own dependencies; the version Home Assistant pins is the one that applies. What the class guarantees: every datetime in and out is timezone-aware and a naive one is refused; the `date` arguments are local dates of the named zone, and the answer is the event that falls on that local date, even where it belongs to another calendar date in UTC; a day without the event (polar night, polar day, an elevation the sun never passes) answers `None`; the elevation of `position` and the passages of `elevation_reached` include the same refraction and agree at any height of the installation, because the passages are computed for an observer at sea level (`astral` lowers the horizon for an elevated observer when it computes a transit, but not when it computes a position); the height of the observer counts for sunrise and sunset alone, where the dip of the visible horizon is the intended effect. Its tests stand in `tests/core/test_sun_astral.py`, because the module needs nothing of Home Assistant and runs on every platform; they check the worked example of the `astral` documentation and made-up round points, never the coordinates of a real installation.
...
| `ResolvedSettings` | The result for one window: one `ResolvedValue` per setting of the registry, every fault on the levels of this window, `disabled_functions` (the functions that are paused, with `faults_of(function)` naming the faults that did it), `group_missing`, `masked_own_values`, and `member_values`: per member and member-level setting the value that applies, with its provenance. |
| `MemberSettingsRegistry` | The member-level settings: a `SettingsRegistry` plus, per key, the window setting a member inherits from. It refuses a setting of a function that falls back, and one with a capability requirement. `MEMBER_SETTINGS` is the one that ships; [Sun geometry and glass calibration](geometry.md#member-level-settings) describes the level, its faults and what H11 builds from it. |
...
| a number outside the `value_range` of its entry | a fault (`invalid`): it can be read, but the setting does not take it. The same range gives the number boxes of the forms their bounds, so it is written down once |
...
Today's settings: `morning_condition_source`, `schedule_profile` and every setting whose key begins with `schedule_` belong to `schedule` ([the schedule](schedule.md#settings) lists them), `shading_temperature_tiers` and the `shading_*` measurements of the geometry to `shading`. `covering_type` names no function (`function=None`): it describes the window itself, not a function. That is allowed only for a setting that cannot be inherited, and a fault in it falls back.

### The resolver

`resolve_settings(registry, capabilities=…, members=…, global_settings=…, group=…, window_settings=…, rules=…)` resolves every setting of a registry. `resolve_window(window_id=…, members=…, …, member_settings=…)` does it for `WINDOW_SETTINGS`, then resolves the [member level](geometry.md#member-level-settings) on top of the resolved window, takes the capabilities from the model (`WindowConfig.capability_states`: over all members, missing beats unknown beats present) and builds the `WindowConfig`.
...
`WindowConfig` has three such rules so far: two of the schedule ([the schedule](schedule.md#triggers-and-clamps)) and one of the glass calibration ([geometry](geometry.md#settings)). Two views of `WindowConfig` end with such rules; a refused rule of the first waits until the single values of the second are checked, so a rule never hides an invalid single value; the generic resolver is also tested with rules over two pausing functions, over a function that falls back, and over one of each.
...
| `stagger_gap` | 2 s | 2 s | staggering applies to protection movements too, and a longer gap would delay them more than the approved default (rule 2) |
...
| `value_range`, `unit` | Left out here: a reference has neither. A number or a duration states its range, `ValueRange(minimum, maximum)` in the stored unit (seconds for a duration; `None` for an open side), and a number its unit (`"%"`, `"lx"`). Take the bounds from the constants of the model that its value rules use (`POSITION_RANGE` for a position, `MAX_RANDOM_OFFSET`, `MAX_STAGGER_GAP`), so that one number is written once. The reader refuses a stored number outside the range, the forms take their bounds and their error texts from it, and `tests/core/test_settings_ranges.py` checks that the value rules of `WindowConfig` take every bound. |
...
2. **The field.** Add the field to `WindowConfig` in `core/model/window.py`, with its default, and validate it in `__post_init__`. The value rules of a setting live there and nowhere else, apart from the range of a number or a duration, which its registry entry states (step 3), from the constants these rules use, or, where the registry is stricter than the model (the brightness threshold is not negative, while the model takes any finite number), as a rule of the registry alone; the resolver hands every set value of every level to `WindowConfig` and turns a refusal into a fault of that key on that level.
...
3. **The one entry.** Add one `SettingDefinition` to `WINDOW_SETTINGS` in `core/settings.py`: `key` is the name of the field, `default` is the default of the field, `kind` is its declared kind, `function` its function, `parse` reads the stored form. Add `inheritable=False` if only a window can set it, `requires=CapabilityRequirement(...)` if it needs a capability, and for a number or a duration its `value_range` and, for a number, its `unit`. Nothing else in `settings` changes: reading stored data, the order of the levels, provenance, the mask, the handling of faults and the construction of the `WindowConfig` follow from the entry.
