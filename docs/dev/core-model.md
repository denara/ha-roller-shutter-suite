...
`WindowConfig` has no such rule yet; the generic resolver is tested with rules over two pausing functions, over a function that falls back, and over one of each.
