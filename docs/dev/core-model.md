...
| `MemberConfig` | One member: its identifier, its capability profile, and `measurements`, what the member states itself for geometric shading (`MemberMeasurements`; by default nothing, so it inherits the window's). The inheritance resolver fills `measurements` from the member-level settings, with sound values only. |
