...
- A named key of a function that **falls back** (only when no pausing key is left to handle, because pausing may already heal the combination): the innermost level that supplies one of these keys is passed by for the keys it supplies, and the build is tried again, level by level, last with the built-in defaults. A test asserts that the defaults alone build.
