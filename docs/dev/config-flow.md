...
The registry of the core (`core/settings.py`) is the only description of a setting: key, kind, function, default, reader, whether it is inherited, the capability it requires, and for a number or a duration its range (`value_range`) and for a number its unit. The forms repeat none of it. A `FieldForm` adds only what the core does not know because it concerns nothing but the form:

| Field of `FieldForm` | Meaning |
|---|---|
| `key` | the key of the registry entry |
| `expert` | the field sits in the collapsed section "Expert values" |
| `step` | the step of the number box of a number or a duration |
| `seconds_per_unit` | the unit a duration is entered in: 1 (seconds), 60 (minutes) or 3600 (hours); it is stored in whole seconds |
...
**The bounds of a number box are the range of the registry, and nothing else.** A `FieldForm` has no bounds and no unit; `tests/ha/test_flow_model.py` fails if it gets any. `Catalog.number_bounds` reads them from the registry entry: a number keeps the range and the unit of its entry; a duration, stored in seconds, gets the whole units of the form that lie within its range (at least 1 second is at least 1 minute) and the unit of `seconds_per_unit` (`s`, `min`, `h`). The same range is the value rule: `settings_from_stored` refuses a stored number outside it as `invalid`, and `tests/core/test_settings_ranges.py` shows that the value rules of `WindowConfig` take every bound. The translation generator writes the error texts from it, so the range is written down once.
...
| `number` | optional number box with the bounds and the unit of the registry entry; empty means inherit; own value as suggested value | required number box |
| `duration` | like a number, entered in the unit of the form, stored in whole seconds | required |
| `time` | optional time selector without seconds; a stored time with seconds loads and is kept | required |
...
- Placeholders of a field: `{<key>_inherited}` and `{<key>_source}`, and `{limited_by_<capability>}` for a stand-in. Their content is language-neutral: numbers with units, entity IDs, names the user chose. A whole number is written without a fraction (`80 %`, never `80.0 %`; `format_number` of the core is the one place), and a time without seconds as `HH:MM`.
- A numbered page gets `{page_number}` and `{page_count}` for its title: the position among the numbered pages of its feature that the flow queued, and their number ("Daily routine: workdays (1/3)"). The title stays a translated text; only the two numbers are placeholders.

### Validation is the core's

`read_step_input` in `flow/inheritance.py` turns the input of a step into stored values and lets the core judge them. **No selector judges first.** Home Assistant validates the input with the schema of the page before the step is called, and a selector of Home Assistant refuses with an English text of its own. The frontend shows that text as it is, because it is no translation key; this is how "Value -100.0 is too small" reached a user: `NumberSelector` turns the input into a float and compares it with its bounds. The generated steps therefore use two selectors that show what the frontend needs and hand every value on unchanged: `NumberBox` (a `NumberSelector` with the bounds and the unit of the registry) and `TimeOfDay` (a `TimeSelector` without seconds). Every refusal then comes from the step, as a translated error key.

1. **A number outside the range** of its registry entry is refused with `out_of_range_<field>`, whose text names the range in the unit of the form ("Enter a value from 0 to 100 %."); for a field in the section the text names the field too. Anything but a finite number is `invalid_value`.
2. **The number selector delivers floats.** A whole float is stored as an `int`. A value with a fraction is kept only if the reader of the setting accepts it; if the reader refuses it but accepts the whole number next to it, the error is `fraction_not_allowed`. A duration is always a whole number of its unit. This is the single place for the normalization.
3. The own values of the level are resolved with the resolver of the catalog. A fault on the edited level becomes a form error: `invalid_value`; for a day of the year `invalid_day_of_year`, or `leap_day_not_allowed` for 29 February, which the core's `as_day_of_year` refuses. A refusal by a rule that spans several settings (`SettingsCombinationError`) becomes `invalid_combination` at every field of the page that is concerned and can show an error, and on the form; the placeholder `{combination}` names the keys of all settings concerned, also those of another level. A field inside the section cannot show an error of its own, so its errors stand on the form. A problem code that this layer does not know is shown as `invalid_value`.
4. The house and a group have no covers. Their values are judged, and their inherited values are shown, through an imaginary window that sets nothing itself and whose one member has unknown capabilities, so nothing is masked (`PROBE_MEMBER`).
...
1. Add the settings to the registry of the core, as [The core model](core-model.md#adding-a-setting) describes; a number or a duration states its range there, and a number its unit.
...
Home Assistant loads one file per language, the strings of a step live under the flow that shows it, and a custom integration cannot use key references. `scripts/build_translations.py` therefore writes `strings.json` and one `translations/<language>.json` per language from `translations_src/`. A language is every `base.<language>.json` there: English is written to `strings.json` and `translations/en.json`, every other language to its own file, and each is compared with English. A new language needs its base file and one fragment per feature, and nothing in the script or the tests changes:

- `base.<language>.json`: everything that is not a feature step, and the shared templates under `_templates` (the inheritance hints, the section of expert values, the errors of the generated steps, the parts of the repair issues about stored settings, and `reason_codes`, the words for every reason code, which the script writes to the states of the reason sensor and to the attribute `reason` of the next planned action). The names of the status entities are under `entity`, the sentences of the logbook under `common`;
