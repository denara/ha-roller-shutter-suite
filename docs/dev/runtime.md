...
| `actuator.py` | the cover actuator adapter: the only place that calls a cover action; dry-run checked a second time, staggering, the result of every call (see [Moving covers](#moving-covers)) |
...
6. **Send.** A decision whose gate outcome is `send` names the target of every member. The controller checks dry-run a second time: a window in dry-run sends nothing, with one exception the project owner decided, a fire wish that was sent because the dry-run rule itself raised (`Decision.faults` names the rule). Then it hands the target of every member that is available and does not stand at its target within its tolerance to the actuator port, together with the decision, under a fresh command identifier (a member that is away or already there is not commanded and gets no record, section 9 of the specification; a member that reports no position cannot be judged and is commanded) and has the core record the send: `Engine.state_after_send(snapshot, decision, command_ids)` is the one recorder of own commands ([The arbiter](arbiter.md)). It writes, per commanded member, the last own command (target, direction, time, wish class, reason) with one attempt at the time of the send, sets the owner of the position to the integration, and for a comfort wish sets the clock of motor protection. The runtime calls it right after each member was handed to the actuator port (which returns at once; the call itself follows in its slot), on the state the decision left behind, with the identifiers of every member sent so far, and stores the result at once: an actuator that raises for a later member leaves the commands that were given recorded, and the next recompute does not send them again. That record is what the gate rules about own commands read on the next recompute, and what survives a reload. The context ID that section 11 lists with the own command arrives with the result of the call ([Moving covers](#moving-covers)): `Engine.on_command_result` writes it into the record, and a `command_failed` result marks the record as failed.
7. **The next wake-up.** The earliest of the times listed above, accepted under the loop rule, and the status.

## Moving covers

`actuator.py` is the only module that calls a cover action (G1). `tests/ha/test_single_mover.py` reads every other module of the integration and fails on a service call through the service registry, a cover action by its name or its constant, a helper that runs an action from a configuration, or a movement method of a cover entity. Keeping it in one small place is what makes dry-run trustworthy.

**The seam.** `CoverActuator` lives in `hass.data`, like the storage, and survives the reload of the config entry. Every controller binds its window when it starts (`bind`, with the window's name, its controls, its staggering gap and a callback for results) and releases it when it stops. The binding, `WindowActuator`, is the actuator port of the core (`move_to(command_id, member_id, target)`); the controller also hands in the decision, which the second dry-run check and the staggering need. A command without a decision is treated as what it cannot prove not to be: nothing is sent for it in dry-run, and it is staggered.

What happens to a command:

1. **Dry-run, checked again.** The adapter reads the window's controls itself, when the command arrives and once more right before the call. A window in dry-run sends nothing, whatever reaches the adapter, with one exception the project owner decided (section 13a of the specification): a fire command whose decision names the dry-run rule among its faults (`Decision.faults`), because the dry-run rule itself raised. An escape route that stays closed in a fire is the greater evil. A refusal is logged as an error with the window's name.
2. **Its slot in a collective movement (E13).** After each motor the next one waits the gap of the window the motor belongs to (`stagger_gap`, default 2 seconds, 0 to 10), across windows and between the members of one window. A window with a gap of zero waits for its turn but reserves no time after its motors, which switches staggering off for them. Fire is never staggered: a fire command is called at once and takes no slot. A newer command to a member replaces a queued one as a whole. The core records the command when it is handed over, so a recompute during the wait sees it as pending and does not send it again; the result of the call names the instant of the call (`CommandResult.called_at`), and for an accepted command the core moves the time of the command, and of its attempt, to that instant. The expectation window therefore starts when the cover was really commanded, and after every result the controller arms its wake-up again from `member_expectation_end`, the one deadline source. The motor protection clock (`last_comfort_movement`) stays at the hand-over, so motor protection judges exactly as without staggering. Two alternatives were rejected: recording the command only when its slot comes (a recompute during the wait would send it again), and adding the queue delay to the deadline inside the adapter (a second deadline source next to `member_expectation_end`).
3. **The action the cover supports**, read from the state of the cover at the moment of the call (`cover_action`): `set_cover_position` where the cover can be set to a position; otherwise `open_cover` for a target from 50 upwards and `close_cover` below 50 (section 8.1 of the specification). A cover that is away or supports neither is not called, and the result is `command_failed`.
4. **The call** goes to the service registry of Home Assistant (`hass.services.async_call("cover", …, blocking=True, context=…)`) under a fresh `Context`, in a task of its own, so a slow or failing cover never holds up another window. The task starts after the controller has recorded the send.
5. **The result** comes back to the controller that runs the window at that moment, also after a reload: accepted, or `command_failed` if the call raised or could not be made, or if a dry-run check refused it (on arrival and right before the call alike). Both carry the context ID and the instant of the call when there was one. The controller hands it to the core (`Engine.on_command_result`), which writes the context ID into the member's last own command, marks a failed one, and ignores a late result of an older command. A result for a window without a controller at that moment is kept, the latest per member, and handed over when the window is bound again. A failure is logged as a warning once per cover, with the window's name, the cover and the type of the error, and not again until a call to that cover works.

**What a failed command changes, and what not.** The record stays: the command is still the member's last own command with its attempt, and it stays pending until its expectation window has closed, as a command the cover did not answer would. Gate rule 3 no longer counts it: a member without position feedback whose command failed is not taken to stand at its target, so it is commanded again once the command is no longer pending and, for a comfort wish, once the minimum interval of motor protection has passed. Nothing here retries earlier; command verification, backoff and the repair issue are a later block.

**Nothing claims an arrival.** A result says only that the actuator accepted the command or that the call failed. On many installations the reported position is calculated from run time, so nothing in the adapter, its logs or this page says that a curtain has arrived.
