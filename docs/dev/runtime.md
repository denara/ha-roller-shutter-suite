...
5. **The result** comes back to the controller that runs the window at that moment, also after a reload: accepted, or `command_failed` if the call raised or could not be made. Both carry the context ID of the call when there was one. The controller hands it to the core (`Engine.on_command_result`), which writes the context ID into the member's last own command, marks a failed one, and ignores a late result of an older command. A result for a window without a controller at that moment is kept, the latest per member, and handed over when the window is bound again. A failure is logged as a warning once per cover, with the window's name, the cover and the type of the error, and not again until a call to that cover works.
