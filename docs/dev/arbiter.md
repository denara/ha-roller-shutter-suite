...
1. **Layers.** Every layer is asked, from fire down to the schedule. The first one that answers with a target or with "leave alone" wins. "Leave alone" wins like a target does: no lower layer acts, and nothing moves. Every other layer ends up in `Decision.other_layers` with a reason: a layer above the winner says why it stepped aside (`inactive`, `input_unavailable` …), a layer below the winner says what it would have wanted (`schedule_night`), a layer nobody registered says `not_configured`, and a comfort layer whose function is disabled for the window because of a data fault is not asked and says `function_disabled_by_fault`. **The record names the function that spoke.** The arbiter writes the function of the registration, never the layer itself, into every entry of `other_layers` (`LayerReason.function`) and, for the winner, into `Decision.winning_function`; a reader of the record never needs the registry to know which function it was. Lower layers are asked although they cannot win, because the status of a window shall be able to say "the schedule wants the night position, but the storm has priority".
...
| 11 | staggering | protection, comfort | not a gate rule: the actuator adapter of the Home Assistant layer spreads a collective movement over time (see [The runtime](runtime.md#moving-covers)); `GateRule.STAGGERING` stays unregistered |
...
- **Target reached (3)** always compares with the real, reported position, also in dry-run. Unavailable members are not judged. A member without position feedback counts as reached if its last real own command had this target, unless that command failed (`OwnCommand.failed`, set from a `command_failed` result): an actuator that never received the command did not move the cover.
- **Movement in flight (8)** has two parts, registered separately because they hold back different classes.
  - *The same command is still pending* (all classes, fire included). An own command is pending while its **expectation window** runs: from the time of the command for the full travel time of its direction plus the report delay of the member (`member_expectation_end` for one member and one command, which `expectation_window_end` of the gate calls; an upper bound and the one place to change when the tracker knows more, so everything outside the gate that waits for the same instant, such as the simulation's runner, calls it too). The window is open while the time is before its end; at the end itself the command is no longer pending. It judges the members a send would command: the members that are available and do not stand at their target within their tolerance (a member without position feedback cannot be judged and always counts). If every one of them has a pending command with its target, nothing is sent again: `duplicate_command`. The runtime commands exactly these members, so a member that already stands where it should, or one that is away, does not make a pending command look like a new one. This hangs on the running window, not on "was sent once". When the window has closed and the target is still not reached, the command is not pending any more: fire is sent again at once, protection and comfort follow their normal rules.
...
- **A layer with two functions is registered in parts, one registration per function.** Shading and solar heating are one layer of the arbiter and two functions: register `LayerRegistration(Layer.SHADING, shading_part, FunctionId.SHADING)` and `LayerRegistration(Layer.SHADING, solar_heating_part, FunctionId.SOLAR_HEATING)`. The rules: the parts of a layer have different functions (the same function twice is refused), each part follows the rule for a registration (a function that is paused on a fault), and only comfort layers have parts. The arbiter asks the parts **in the order in which `FunctionId` defines its members, never in the order of registration**; that order is documented at the enumeration and pinned by a test. The first part with an opinion is the wish of the layer. Every part is asked, as every layer is, except a part whose function is disabled for the window: it is **never called**, so code with faulty settings does not even run, and no layer has to police itself. Each part speaks for itself in the record: `Decision.other_layers` has one entry per part that did not win, with the function that spoke. If both parts have an opinion, the documented order decides, and the part that lost is an entry with the reason of its own wish; a skipped part is an entry with `function_disabled_by_fault`, also when another part of the same layer won, so the fault stays visible in the decision. A carry-over for the block that builds shading and solar heating: it owes a test that the two never have an opinion at the same time with a valid configuration.
...
| a gate rule | **the rule holds the wish back**, a protection and a comfort wish always, a fire wish only if the rule is the maintenance lock (see below). It is a deferral with `reevaluate_no_later_than`, the upper bound of every deferral whose end nobody knows (`WindowConfig.reevaluate_after`), so the window is evaluated again | `GateOutcome` with `gate_rule_failed` and the rule |
...
**Fire and protection are never stopped by this.** The fire layer and the protection layers are asked even if every comfort layer raised. A fire wish is subject to no constraint and is never asked the rules of the fire bypass, so whatever those raise cannot reach it. For the rules the bypass does **not** skip, the project owner decided: **only a failed maintenance lock holds a fire wish back.** `_failed_rule_lets_pass` in `arbiter/arbiter.py` is the one place that holds it:
...
| 2 no member can execute | **sent**: a command to a member that cannot execute it costs nothing; an escape route that stays closed costs a life | held back | held back |
| 3 target reached | **sent**: at worst a shutter that is open already is told to open | held back | held back |
| 8 movement in flight, the part about a command that is still pending | **sent**, but never twice inside one expectation window, see below | held back | held back (both parts) |
...
| 4 to 7, 9 to 11 and the deferral of 8 (the fire bypass) | never asked, so it passes | held back if the rule applies to protection | held back |

**A fire command is still never repeated inside its expectation window.** A rule that fails at every recompute must not send at every recompute, and the rule that fails may be the very one that suppresses duplicates. So the safety net guarantees it itself: when a fire wish has passed a failed rule and no other rule holds it back, the arbiter sends only if no own command of class fire with the same targets is inside its expectation window. It reads that from the persisted, **real** own commands (`fire_command_pending` in `arbiter/gate.py`), without calling any rule, and answers `duplicate_command` otherwise. When the window has ended and the alarm is still active, the command is sent again, as on the normal path. It reads the real commands also for a window in dry-run: what passed a failed dry-run rule was really sent. A window in dry-run whose dry-run rule works is not affected: a fire wish that passed another failed rule reaches the dry-run rule, the record reads "would have sent", and the would-be command is remembered once, exactly as without the failed rule.

A failed dry-run rule records no would-be command, because nobody knows what it would have recorded; `GateOutcome.dry_run` still says whether the window is in dry-run. A fire wish that is sent although the dry-run rule failed is the outcome `send`; `Engine.state_after` leaves the state as it is. For the authors of the Home Assistant layer: the adapter checks dry-run a second time before every command; for a fire command of a decision whose `faults` name the dry-run rule, that second check must follow the same ruling, otherwise it undoes it.

A protection wish is "still decided" in this sense: its layer is always asked and the decision names it as the winner with its target, so the event can be fired. A constraint or a gate rule that applies to protection and raises holds it back like a restriction that applies, and it is tried again after `reevaluate_after`.
...
## The two generic tests of the fault values

A setting of a function that falls back states a fault value in the settings registry ([Fault values](core-model.md#fault-values)). Whether that value is cautious can only be seen in a decision, so two tests run the arbiter over the registry. Neither names a setting: `tests/core/test_fault_values.py` walks `WINDOW_SETTINGS` and the situations of `tests/core/fault_value_situations.py`, **the one place** where the situations stand.

1. **The mandatory test** (`test_faulty_value_never_leads_to_a_decision_a_valid_value_would_have_forbidden`): for every setting of every function that falls back, in every **comfort** situation of its function, the window is resolved with the setting faulty (its stored value cannot be read on the only level; and the house level unreadable as a whole, where every fault value applies at once) and decided. That decision never **sends more** than the decision with a valid sample value: it moves no member that the other leaves alone, and none further in the same direction. The samples are, by kind: always the default and what the situation sets; both positions of a switch; "none" and a working source for an optional reference.
2. **The protection test** (`test_fault_value_never_restricts_a_protection_wish_more_than_the_default`): for every such setting, whatever its function, in every **protection** situation, the protection wish decided with the fault value never sends **less** than the same wish decided with the default. A data fault must not hold back, limit or stop a hail opening.

**What they prove, and what not.** Test 1 proves that a fault never makes a function less restrictive for a comfort wish than the default, than either position of a switch, or than a working or an absent source, in the listed situations. It does **not** prove that the fault value is at least as restrictive as every number somebody could have meant: a threshold, a position and a duration have no most restrictive value that would still be useful, so for them "cautious" means "never less restrictive than the default". For `reevaluate_after` no value is less restrictive than another, so the test covers it without being able to fail on it. Both tests say nothing outside the listed situations. **The scope of test 2** is a fault **on its own**: it compares with the default while `frost_applies_to_protection` has its default or its fault value, which are both "off", and `test_scope_of_test_two_is_the_default_of_applies_to_protection` fails if a listed situation switches it on. Where a person has **validly** switched on that frost protection restricts protection wishes, the cautious values of the other frost settings reach those wishes exactly as their valid restrictive values would. Named tests in the same module document that other side: with a valid "applies to protection", a working source, frost, a closed shutter and hail, a faulty `frost_hold_closed` keeps the shutter closed exactly as a valid "on" does (its default "off" gives 90); a faulty `frost_source` limits the opening to 90 exactly as a valid source in frost does; and a fire wish opens untouched in each of these situations. They judge the restriction, not the wish: an unreadable level also pauses the comfort functions, which has its own tests, and the situations keep the wishes so that the restriction stays visible.

**The tests notice a wrong value**, and that is tested too: `test_less_restrictive_fault_value_fails_the_mandatory_test` changes one fault value of a copy of the registry at a time (the frost source to "none", the frost position to 100, "hold closed" to off, the threshold to −1, the hysteresis to 0.2, the minimum change to 2, the minimum interval to 5 minutes) and expects findings, and `test_fault_value_that_restricts_protection_fails_the_protection_test` does the same for the protection test with "frost applies to protection" switched on.

**How a later block adds situations.** A block that registers a setting of a function that falls back (C08: the contacts of lockout protection and of the ventilation floor) writes no new test. It adds `Situation` entries to `SITUATIONS`: a world in which its function restricts a comfort wish, and one with a protection wish if the function can restrict one, each with the few valid settings that make the function restrict. If its constraint or gate rule is not built in, it adds it to `arbiter()` in the same file. Until the function has a situation, `test_every_function_with_settings_that_fall_back_has_a_situation` fails and says what to add where; and `test_situations_tell_the_values_of_a_switch_or_a_reference_apart` fails for a switch or a reference whose values no situation tells apart, because the tests would then say nothing about it.
