...
| Spike code | Branch `spike/s2-config-flow`. It is never merged. Its tests are `tests/ha/test_s2_*.py`; run them with `uv run pytest tests/ha` as described in [testing.md](testing.md). |
| Date | 2026-09-20 |
...
**Brief and architecture document on N3.** The brief says "A window has exactly one cover; that cover may be a Home Assistant cover group" (N3). The architecture document, section 9, decides for one or more member covers, resolves a cover group into its members and stores members only, and notes that this changes the wording of N3, "a change to the brief for the owner to make". The spike follows the architecture document, as instructed for this block. The brief still has the old wording; H01's block file already points to the architecture document for this. The owner should update N3 so the two documents agree.

**Members with their own values (architecture document, section 9, decision 9)** add a third inheritance step *inside* a window: a member inherits the window's measurements unless it states its own. The spike did not build this. The recommended pattern covers it without change (a member's form is one more inheriting level whose parent is the window), and a flow can show the same step repeatedly, once per member, with the member's name as a placeholder. This is a proposal, not a tested result; see below.
