...
> 5. **A level that is unreadable as a whole** counts as not present: all pausable functions of the affected windows pause, also for a window that overrides some keys, because it is unknown which keys would have been concerned; protection runs with the values of the next level. The principle of rule 1 holds here too: nobody can see any more whether the unreadable level had set a value, so for every setting of a function that falls back **that the unreadable level could have held (an inheritable setting)** and for which no other level supplies a valid value, the cautious fault value applies, not the default. A setting that can be set on the window only (a lockout contact, a window contact) is not affected by an unreadable house or group level; otherwise one broken house level would stop every window from closing.
>
