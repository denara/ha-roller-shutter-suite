...
> 5. **A level that is unreadable as a whole** counts as not present: all comfort functions of the affected windows pause, also for a window that overrides some keys, because it is unknown which keys would have been concerned; protection runs with the values of the next level.
>
