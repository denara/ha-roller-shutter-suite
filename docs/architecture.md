...
> **Exceptions inside the arbiter follow the same direction.** A programming error must not be able to do what a data fault may not. An exception in a **comfort layer** counts as "no opinion" of that layer and is reported. An exception in a **constraint or a gate rule** counts as its cautious result (the restriction applies) and is reported. Neither ever stops a fire or a protection decision: the fire layer and the protection layers are evaluated, and their wishes pass the gate, whatever a comfort layer, a constraint or a gate rule that the fire bypass skips raised. Two rules are never skipped by the bypass, and they differ. A failed **maintenance lock** holds every wish back, fire included, because it protects a person working at the shutter. A failed **dry-run rule** holds back every wish except fire: with a fire wish pending the command is sent, because an escape route that stays closed in a fire is the greater evil than a test window that opens on a fire alarm.
