...
| 11 | Staggering (E13) | protection and comfort | Defer by the window's slot in a collective movement. This rule is realized in the cover actuator adapter of the Home Assistant layer (block H03), not as a registered gate rule: one window's arbiter cannot know its slot in a collective movement, and a gate rule could only defer by a time that depends on the other windows. The adapter spaces the calls per motor, across windows and between the members of one window, never for fire; the expectation of a staggered command starts at its actual call (section 8.3). Decided by the project owner on 2026-09-24. |
...
- **Own command:** remember target, direction, time, wish class and the context ID; enter `expecting`. The time of the command is the instant of the actual service call, which the actuator adapter reports with the result; until the result arrives the record carries the instant the command was handed to the adapter. The time of the last attempt for the command backoff (section 11) moves to the call as well. The motor protection clock (the time of the last own comfort movement) stays at the hand-over. Decided by the project owner on 2026-09-24 with block H03, because a staggered command would otherwise lose its waiting time from the expectation window. No expectation is created for a command whose target equals the current position (the gate stops those).
- **Deadline** of the expectation: `report delay + start allowance + travel time of the direction × share of the travel × slack + end allowance`. Defaults: start allowance 10 s, slack 1.5, end allowance 5 s; the report delay comes from the capability profile. The proportional share alone underestimates movements that end in an end stop, and a fixed start allowance alone would raise a false "did not react" on a polled platform. Command verification (N1) and every other timer that waits for a report use the same deadline; none of them uses a fixed number of seconds. The deadline is computed by one function of the core (`member_expectation_end`, block C05), which the gate, the runtime and the time-lapse simulation all call, so that a later change of the formula reaches every waiter at once.
...
**Restart reconciliation** is a pure function of (persisted state, live observation) with one of these results: `first_setup` (nothing persisted: owner `unknown`, no dam); `unchanged` (the live position matches the last observation or the last own target within tolerance); `moved_during_downtime` (it does not: owner `user`, the manual override dam is armed with the default end rule); `expectation_expired` (an own command was pending: handled as `movement_not_finished`). Dams and episodes whose end lies in the past are dropped. After reconciliation the window is recomputed, and no decision is made before its members are available. For a window with several members the runtime waits for all of them, but not without end: once at least one member is available and a **start-up grace** (default 2 minutes, one constant of the runtime) has elapsed, the window decides with what it knows. The grace follows the measurements of spike S1, where polled cover platforms reported up to a minute late; a member that has still not reported stays unknown, with no assumed position and no assumed capability, and the gate treats it as unavailable (section 9). Recorded on 2026-09-23 as a deviation from the letter of the previous sentence, decided by the project owner with block H02.
...
>
> 1. **What falls back never fails, and a fault never loosens it.** A faulty value of a function that falls back never makes that function less restrictive than a valid value would. The faulty value is skipped. If another level supplies a valid value, that value applies. Otherwise the **cautious value** of the setting applies, not its default. For that purpose every setting of a function that falls back declares a **fault value** next to its default in the settings registry. Often the two are the same (a threshold, a position, a duration). They differ where the default means "not configured" or "off": for an **optional reference to a source** the fault value means "configured, but blind", and the rules for blind sources apply (the frost limit stays; an unknown contact never leads to closing); for a **switch** the fault value is the more restrictive of the two, within one limit: **a fault on its own never restricts a protection wish more than the default does; cautious fall-back values may restrict comfort only.** Where the user has validly switched on that a function that falls back restricts protection wishes (here "frost applies to protection"), the cautious fault values of its other settings reach those wishes exactly as their valid restrictive values would: on such a window a faulty "hold closed in frost" keeps a closed shutter down in frost also against a hail opening, as a valid "yes" would, and a blind frost source limits the opening to the frost position, as a valid source in frost would. Fire stays untouched in every case. Where two aims of protection stand against each other, the order decided for the defaults holds in the fault case too. Example: "frost applies to protection" keeps its default "no" as fault value, because otherwise an unreadable house level would, together with the blind frost source, stop a hail opening of a closed shutter all year round; "hold closed in frost" takes "yes", which concerns comfort only. A second test goes through every setting of every function that falls back and shows that a protection wish decided with the fault value is never restricted more than with the default. Fire and protection events keep running for every window, and no restriction is ever lifted by a data fault. A test goes through every setting of every function that falls back and shows that a faulty value never leads to a decision that a valid value would have forbidden. *Reason for the fault value (found in the review of the arbiter):* an unreadable frost source on the only level that set one fell back to the default "no source", which means "frost protection not configured", and the frost limit disappeared: a data fault led to more movement.
...
> 5. **A level that is unreadable as a whole** counts as not present: all pausable functions of the affected windows pause, also for a window that overrides some keys, because it is unknown which keys would have been concerned; protection runs with the values of the next level. "All pausable functions" means the pausable functions whose settings the unreadable level can hold at all, as the pause applies only where the faulty value would have been effective: for the house, a group or a window that is every pausable function; for a member level, which holds only the measurements of shading (decision 9, block C09), it is shading alone. The principle of rule 1 holds here too: nobody can see any more whether the unreadable level had set a value, so for every setting of a function that falls back **that the unreadable level could have held (an inheritable setting)** and for which no other level supplies a valid value, the cautious fault value applies, not the default. A setting that can be set on the window only (a lockout contact, a window contact) is not affected by an unreadable house or group level; otherwise one broken house level would stop every window from closing.
>
...
> **Exceptions inside the arbiter follow the same direction.** A programming error must not be able to do what a data fault may not. An exception in a **comfort layer** counts as "no opinion" of that layer and is reported. An exception in a **constraint or a gate rule** counts as its cautious result (the restriction applies) and is reported. Neither ever stops a fire or a protection **decision**: the fire layer and the protection layers are always evaluated, whatever a comfort layer raised, and the decision names their wishes. What happens to the wish then depends on its class. A **protection** wish is held back by a failed constraint or gate rule that applies to protection (the operating mode, the person-at-the-window dam), exactly as if the restriction applied, and is tried again after the re-evaluation time; an exception may not loosen for protection what a data fault may not. A **fire** wish is never asked the rules its bypass skips. Of the rules the bypass does not skip, only a failed **maintenance lock** holds fire back, because it protects a person working at the shutter. Every other failed rule sends a pending fire wish: the dry-run rule, "no member can execute", "target reached" and the pending-command part of "movement in flight". An escape route that stays closed in a fire is the greater evil than a test window that opens on a fire alarm or an opening command that was not needed. The suppression of duplicates over the expectation window stays effective for such a fire command, guaranteed by the safety net itself, so that a rule that fails permanently does not repeat the command at every recompute. For protection and comfort a failed rule holds the wish back.
...
| 15 | Faulty stored settings | One rule on all levels: functions that protect or restrict movement fall back to the next level and never fail, and a fault never makes them less restrictive (cautious fault value instead of the default); a faulty setting of a function that creates comfort wishes pauses that function for the windows for which the value would have been effective; everything is reported; unknown keys are reported but harmless |
