...
- **Whatever protects or holds a shutter back keeps working.** Fire, storm, frost protection, motor protection, the rule that a shutter stays partly open in front of a tilted window, and everything else that protects people or the hardware or limits a movement never stops because of a faulty setting. The faulty value is skipped and the value of the next level is used: the group's instead of the window's, the house's instead of the group's.
- **A faulty setting never loosens protection.** If no level has a valid value either, the integration does not simply take its built-in default, because for some settings the default means "switched off". It takes the **cautious value** of that setting instead. An example: the temperature sensor for frost protection is stored for the house, and that entry becomes unreadable. "No sensor" would mean "no frost protection", and your shutters would open fully into a frozen end stop. Instead the integration treats the sensor as configured but silent, keeps opening only up to the frost position, and tells you so. For most settings (a threshold, a position, a waiting time) the cautious value is the built-in default. Two things it never does: a faulty setting never makes a limit weaker than any valid value would have, and a faulty setting on its own never holds back a movement that protects, such as opening the shutters when hail is coming.
...
If the settings of a whole level cannot be read at all, the integration cannot know which settings are concerned. It then pauses all convenience functions for the windows of that level, and everything that protects or holds back carries on with the values of the other levels. Nobody can see any more whether the unreadable level had set such a value, so where no other level has one, the cautious value applies here too. A group that was removed is not such a case: as described above, its windows simply take their values from the house, and nothing pauses.
...
- **Frost protection.** While it is freezing, the integration opens a shutter only up to the frost position (90 % by default) instead of fully, so the shutter does not run into a frozen end stop. Closing is never limited. You can lift this for a window when you know the shutter is free. If the temperature sensor stops reporting, the integration keeps what it last knew for a day. After that it does not assume "no frost": it applies the limit as a precaution, says so in its record (`frost_limit_source_blind`), and carries on normally as soon as the sensor reports again or you lift the limit. The integration cannot detect a shutter that is frozen in place; it can only avoid the movement that does the damage. Every frost setting (the temperature source, the threshold, the frost position and so on) can be set for the house, for a group or for a single window, like any other setting. Frost protection and motor protection restrict movement, so a faulty stored value never switches them off and never weakens them: the valid value of another level applies, and if there is none, the cautious value of the setting, and the fault is reported. For frost protection that means: a temperature source that cannot be read from the settings counts as a sensor that is silent, so the limit stays; and a faulty entry for "do not raise a closed shutter in frost" counts as switched on. A movement that protects, such as opening for hail, is not limited because of a faulty frost setting, unless you yourself have switched on that frost protection also applies to such movements; then the cautious values act there just as your own strict settings would. Opening for a fire alarm is never limited by any of this. A sensor that delivers text instead of a temperature counts as silent too.
