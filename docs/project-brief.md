...
| License | MIT |
| Next step | Implementation along `TASKS.md`. The domain design specification [architecture.md](architecture.md) is approved and refines this brief where the brief says so |
...
- **G3 — Incremental rollout.** Windows can be added, put into dry-run, armed and removed individually without affecting what the others do. Technically Home Assistant reloads the whole config entry on every configuration change, and with it every window; the runtime state of each window (a movement in flight, overrides, deferrals, episodes) therefore survives a reload through persistence, and a reload never causes a false manual detection or a duplicate command.
...
| A9 | Sleep / night mode | Switch, global and per window or room. Closes, blocks shading and solar heating, replaces fixed night time windows. Switching it on is a deliberate act and ends a manual override of the windows it covers; a movement by hand during sleep mode is respected as usual. | Will be implemented | [BP], [#12] |
...
| A12 | Frost protection | Preventive: at frost the integration opens only to a frost position (default 90 %), so a curtain does not run into a frozen end stop. Closing is never limited; "do not raise at all" is a separate option that is off by default. The integration cannot detect a curtain that is frozen in place. The frost source is freely selectable and inherited, so it can differ per group or façade, because a regional value says little about a sunlit wall. The operator can waive frost protection per window, per group and globally until the next morning; the waiver is persisted, visible in the status and fires a warning event. A movement by hand is never limited, so the operator can test whether a curtain is actually stuck. Optionally the limit is lifted for a window that has been in direct sun for a configurable time, with a warning event. By default it limits comfort movements only; whether it also limits protection movements is configurable. Fire (D3) always ignores it. | Will be implemented | [#24], project owner |
...
| B2 | Lockout protection | An open door (terrace, balcony, insect screen door with its own contact) blocks every closing movement, including evening, night and storm. Closing is caught up once the door is shut. Several blocking contacts per window. A blocking contact that is unavailable counts as open; because nothing closes at that window then, not even at storm, this is reported as a repair issue and an event after a configurable time. | Will be implemented | [BP], [#3] |
...
| D3 | Fire alarm | Smoke detector triggers: all shutters open (escape routes). Overrides sleep exceptions, pauses, staggering, motor protection, frost protection, the manual override and every operating mode including "off". Only two things stop the movement: the maintenance lock (E4) and dry-run (E11). In both cases the fire event is still fired immediately so the situation never goes unnoticed. **No automatic return** after the alarm ends; a human decides by acknowledging it. Between the end of the alarm and the acknowledgement the integration holds all other logic back but moves nothing, so after a false alarm it does not reopen a shutter that somebody closes by hand. | Will be implemented | [#4] |
...
| D6 | Robust protection logic | A running event is detected and caught up after a restart. `unavailable` and `unknown` are neither a warning nor an all-clear: they do not trigger and do not release. A protection that has lost its source must not stay silent: after a configurable time a repair issue and an event say that the protection is blind; its behavior does not change. | Will be implemented | [#18], [OWN] |
...
| E3 | Grace period after own movement | Position reports during and shortly after a self-initiated movement do not count as manual. Travel time per direction is a configuration value, never learned from reports; the cover's `opening`/`closing` state is used where available. Some platforms are polled and report up to a minute late, without any transit state; the expected report delay is part of what is known about a cover, and every timer that waits for a report follows it. This is the primary mechanism of E1 (guardrail 6). It must also hold for covers that never report `opening`/`closing`, that report their final position late, or that settle a few percent away from the commanded position. | Will be implemented | [#6] item 3 |
...
| E10 | Motor protection | Minimum change (blueprint: below 5 % no movement) and minimum interval between comfort movements. Does not apply to protection movements. End positions are exempt from the minimum change, and a fresh wish (a schedule time, a switch, a new request, the start of a shading episode) is exempt from the minimum interval, which exists against flapping while a position is tracked. Motor protection caps no total: comfort movements per window and day are counted, shown in the diagnostics and, above a configurable threshold, reported once a day without blocking. A hard daily limit is not part of the first version, see the outlook. | Will be implemented | [BP] |
| E11 | Dry-run | Per window the integration decides and logs but does not move. Enables side-by-side migration on a productive system. Dry-run never moves anything, not even at fire (D3): it only records what it would have done, and the record shows the complete hypothetical outcome: what would have been sent, or which rule would have held it back. Movements of the other controller are logged but never count as manual operation. New windows start in dry-run and are armed by a deliberate step. During side-by-side migration the old system still controls the same window, and two controllers must never act on it (section 6). | Will be implemented | project owner |
...
| ID | Feature | Summary | Status | Source |
|---|---|---|---|---|
| F1 | Actions for automations | Public actions such as "request position with reason", "clear override", "recompute target state", "acknowledge fire alarm" and "reference run" (a full movement into an end position, so that an actuator that only calculates its position counts from a known point again). Basis for A11 and for scenes. A request ranks below sleep mode: an automation that wants to open a window in a room with active sleep mode, such as an alarm clock, has to end that room's sleep mode first. | Will be implemented | project owner |
...
| ID | Feature | Summary | Status | Source |
|---|---|---|---|---|
| N1 | Command verification | After a movement command the integration checks that the actuator reacted. On many installations the reported position is calculated from run time, not measured: the actuator reports the target as reached even if the motor cut out. Verification can therefore establish only that an actuator did **not** react (unavailable, no movement reported, no change of position), never that a curtain arrived, and nothing in the integration claims otherwise. Whether a cover's position is measured or calculated is stated by the user; calculated is the default. A command without effect is retried with backoff and, if it keeps failing, reported as a repair issue and a reason event. After a blocked or interrupted movement a calculated position can be wrong until the next end position; the integration marks it as uncertain and says so in an event. A cover that settles slightly off the commanded position must not cause a command loop. | Will be implemented | planning review, related work |
...
| N3 | Window and cover cardinality | A window has one or more covers that are operated as a unit: commanded together, observed individually, with one device and one status for the user. The covers of a unit may differ in size, travel time and glass measurements. If the user selects a Home Assistant cover group, configuration resolves it visibly into its members; the group entity itself is not used, because it reports only the mean of its members. A cover belongs to at most one window; configuration validates this per member and explains a conflict. | Will be implemented | planning review, project owner |
| N4 | Removing windows and groups | Removing a window deletes its device, entities and persisted state (episodes, overrides, remembered positions). Home Assistant offers no way to prevent the removal of a group that windows still reference, so a defined fallback applies: such a window inherits from the house level, a repair issue names it, and saving the window's form repairs the reference. No orphaned reference leads to an error. | Will be implemented | planning review |
...
1. **One arbiter instead of competing rules, in three stages.** One arbiter per window decides; no feature moves a cover on its own. A single priority ladder with one winner is not enough: several rules are not position wishes at all (lockout protection only blocks closing, frost only limits travel, the maintenance lock forbids movement), and the sleep-room exception (D4) would let a lower entry beat a higher one. The arbiter therefore has three stages:
   ...
   - **Constraints** limit the winning wish without replacing it: lockout protection (B2, void while tamper is active, F6), an open or tilted window as a floor under the comfort wishes (B1, B4, B9), evening moves downward only (A6), frost protection (A12), no intermediate position during storm (section 6), the sleep-room exception (D4).
   - **The gate** answers "may the integration move now?": maintenance lock, operating mode, pause, manual override, a movement already in flight, motor protection (E10), staggering (E13), command backoff (N1), and dry-run (E11) as the last barrier, so that a window in dry-run still shows what would have happened. The gate sends, defers until a point in time, or suppresses, always with a reason.
...
2. **Decided rules of the arbiter.** The exact layer order, the list of constraints and gate rules, and the reason codes are defined in the domain design specification [architecture.md](architecture.md), which the project owner has approved. These rules are fixed:
   ...
   - **"A person at the window wins" (guardrail 3) needs a second, time-limited dam.** The override dam lets protection layers pass, but a person at the window shall win against storm and hail for a limited time. This second dam also holds protection layers back, ends by itself after the configured time, and never holds back fire. How the two dams relate (what arms each, whether one can turn into the other, what happens when the time-limited dam ends while the override is still active) is defined in the domain design specification.
   ...
   - **Order of the layers** (highest first): fire → hail → storm and other protection events by rank → sleep / night mode → requests from automations (F1) → privacy (F2) → shading and solar heating → schedule. Maintenance lock, lockout protection, window interaction, operating mode, pause, the manual override and "a person at the window" are not part of this order; they are gate rules or constraints as described above.
...
- Tests with `pytest-homeassistant-custom-component` against the current Home Assistant version (2026.9.2 at the time of writing), deprecation warnings treated as errors. Home Assistant reports deprecated usage almost only through its log, so the tests also fail on any such log message that names this integration. The integration must not use deprecated functionality and must not cause a deprecation message; this is demonstrated for the tested Home Assistant versions and the code paths the tests reach, which is why coverage has a threshold and a scheduled run checks the newest Home Assistant release. Python 3.14 via `uv` is required for current Home Assistant versions (precisely: 3.14.2 or newer, see above); `asyncio_mode = "auto"` [OWN].
...
- **An optional hard limit of comfort movements per day** (off by default), once the pilot installation shows what a sensible number is. Tracked shading exceeds twenty movements on an ordinary sunny day, so a limit chosen without data would freeze it in the afternoon.
...
The points that this brief left to the domain design specification are settled there: layer order, constraints and gate rules with the fire bypass; the two dams; the state-based schedule without catch-up; episodes and parts of the day; the source of the season; the watchdog; override duration during a protection event; the inputs of F2; the remaining vague wording of C4, F3, E13 and D9; day types from entities chosen by the user. See [architecture.md](architecture.md), section 15 for the list of decisions.

Still open:

1. School holidays as a day type (A3): calendar entity or dedicated source. Not part of the first version.
