...
- **A faulty subentry never brings the config entry down.** Stored data of a window or a group that cannot be read (a `null` where a key should be absent, an unknown key, a wrong type, a reference that does not resolve) must never make the set-up of the whole config entry fail. Only the affected window is not set up; a repair issue names it and says what is wrong; every other window is set up and runs. A faulty group is treated like a removed one: its windows inherit from the house level, and a repair issue names the group. Reason: the protection and fire functions of all other windows must not depend on a single stored entry. The repair issue disappears when the subentry has been saved again through its form or removed. In this block: reading and validating stored subentries is isolated per subentry, and the repair issues exist; tests create an entry with one valid and one faulty window (and one with a faulty group) and check that the entry loads, the valid window's device exists, and the issue names the faulty one.
