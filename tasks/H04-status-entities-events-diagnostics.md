...
- **Diagnostics download** for the config entry and per window device: resolved configuration with provenance, capability profile, last decision records, persisted state; entity IDs and names of the installation are redacted or clearly a user's own data, following the diagnostics guidance. A decision record carries `Decision.faults` (block C03a): the stage that failed and its reason code are shown, the exception object in `EvaluationFault.exception` is never serialized, because an exception message can contain an entity ID.
- **Added after block C03a:** the three reason codes of the arbiter's safety net (`layer_failed`, `constraint_failed`, `gate_rule_failed`) have no translation yet; this block adds them like every other code. The repair-issue text `fault_consequence` (in `translations_src/`) says that protective functions keep running "with the value of the next level"; since C03a they may run on the cautious value of a setting that no level set. Reword it, and decide together with the project owner whether the one issue of an unreadable level lists the settings that now run on cautious values (the core reports each of them with the action `fell_back_to_cautious_value`; block H01's `windows.py` raises no separate issue for those entries).
- **From H02:** `SourceReading.since` (how long a condition has held, for the status) restarts with the controller at every reload; the core's own persisted instants (for example the brightness instant of the schedule) do not. The status must not present `since` as a persisted fact.
