...
- **Recompute triggers:** state changes of the entities a window uses, points in time the core asked for ("defer until", next schedule change), a periodic safety tick, and an explicit request. Recomputes of one window are serialized; a burst of changes is coalesced.
