...
- **Capability mask for inherited values.** A group cannot know what the covers of its windows can do, so a window can inherit an option that its own cover cannot execute (for example hold-to-move for a cover without stop, or a shading position for a cover without set position). The resolver therefore takes the window's capabilities (lowest common denominator of its members) and masks the resolved result: a setting that requires a missing capability resolves to "not available", with the provenance kept and the missing capability and the limiting member named, so that the UI (block H12) can explain it and the arbiter never acts on it. **A capability has three states: present, missing, unknown.** If a member's entity is not available at start or for a while, its capabilities are unknown; then the last known state applies (the Home Assistant layer provides it), and the resolver neither masks nor reports. For a window with several members: missing if any member definitely lacks the capability, otherwise unknown if any member's is unknown, otherwise present. **A window's own value for such a setting is masked and reported in the same way; it never makes the window invalid.** The own value stays stored and takes effect again as soon as the capability is back. Reason: the protection and fire functions of a window must not depend on one setting that stopped fitting after a cover was replaced. Which setting requires which capability is declared with the setting, in the same single place as its other properties.
- **"Explicitly none" for optional references.** Stored data says "inherit" by an absent key, so a window could not say "none" for an optional source while its group names one. For exactly this there is one documented marker, a string constant, accepted only for settings whose declared kind is "optional reference". The single conversion from stored data knows the marker and delivers the set value "none"; the marker on any other kind of setting is a fault; `null` stays a fault; the marker never reaches the resolved configuration.
...
- A value inherited from a group is masked when the window's capabilities do not allow it; the result names the capability and the member, and the provenance still shows the group. A window's own value for such a setting is masked too, stays stored, and is effective again when the capability returns; the window keeps its configuration. An unknown capability never masks and never reports, and a sequence present, unknown, present produces no report at any point.
