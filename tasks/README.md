...
- **Every push is checked by the pre-push hook** (`docs/dev/contributing.md`, "The pre-push hook"). Run `git push` plainly, without a pipe or a formatter behind it, and read its exit status. The hook prints one line that starts with `instance data:` (the checkout) and one that starts with `instance data, pushed commits` (every commit the push sends, judged one by one, including its message and the addresses of author and committer). **If either line is missing, the push was not checked: stop and report.** A refused push is wanted behavior: never bypass the hook (`--no-verify`, `-c core.hooksPath=`), never change git configuration; if the hook is older than your branch's guard or the other way round, merge `origin/main` into your branch first. If the hook flags a commit of your history, do not rewrite anything; stop and report. State the numbers of both lines of your first push in the pull request, in your own words. Before the first push of a block, look at `git status` and at the list of files the branch adds: no symbolic links, no file outside the folders the block names, no file or folder name that looks like an entity ID, a host, an address or a user name.
