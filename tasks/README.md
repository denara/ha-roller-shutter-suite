...
- **Every commit stands on its own:** it imports, and the tests that exist at that commit pass. The project owner merges with a linear history, so every commit of your branch ends up on `main`, and a commit that only works together with a later one breaks `git bisect`. If you notice such a commit after pushing, say so in the pull request, so the owner can squash.
