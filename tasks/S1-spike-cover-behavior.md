...
**Closed for the design.** History data and two approved active tests are evaluated; the findings are part of `docs/architecture.md` ("Observing a movement", "Several covers operated as one window"). On one platform the reported end position equals the commanded target in every movement with a known target; the other reports only on a grid of about 60 seconds when several members move at once, which is why the capability profile has a report delay. Not measured: a second command during travel and the real command latency; both are covered by design assumptions in the architecture document, and the tracker records latency, report delay and end deviation in the diagnostics. Blocks C06 and H15 are not blocked by measurements any more. The text below is the earlier state.
